...
(() => {
  ...
  // Polled approach: try to init every 500ms until found or 5 tries reached
  let typeAttempts = 0;
...
(function forceTypewriter() {
    const start = () => {
        ...
        const words = [
            ...
            'Reactive-Deliberative Hybrid • Temporal Workflows • Enterprise Automation',
            'Neuro-Symbolic • Policy-as-Code Governance • Financial Compliance',
            'LLM Engineering • Model Optimization • RAG Augmentation'
        ];
        ...
        function step() {
            ...
            if (!deleting) {
                charIndex++;
                inner.textContent = current.slice(0, charIndex);
                if (charIndex >= current.length) { deleting = true; setTimeout(step, 2000); return; }
            } else {
                charIndex--;
                inner.textContent = current.slice(0, charIndex);
                if (charIndex <= 0) { deleting = false; wIndex = (wIndex + 1) % words.length; }
            }
            setTimeout(step, deleting ? 25 : 45);
        }
        step();
    };
...
(function diagnosticTypewriter() {
  const words = [
    'TEST: Systems in the Real World.',
    'TEST: Real Problems, Real Constraints.',
    'TEST: How Ideas Survive Production.'
  ];
  
  function start() {
    const el = document.getElementById('typewrite');
    if (!el) {
      console.error('Typewriter Test: #typewrite element not found');
      return;
    }
    ...
    function step() {
      ...
      else if (isDel && cIdx === 0) { isDel = false; wIdx++; speed = 500; }
      
      setTimeout(step, speed);
    }
    step();
    console.log('Typewriter Test: Started successfully');
  }
...
(function() {
  function init() {
    ...
    function step() {
      const cur = words[wIdx % words.length];
      inner.textContent = isDel ? cur.substring(0, cIdx--) : cur.substring(0, cIdx++);
      
      let speed = isDel ? 25 : 45;
      if (!isDel && cIdx > cur.length) { isDel = true; speed = 2000; }
      else if (isDel && cIdx === 0) { isDel = false; wIdx++; speed = 500; }
